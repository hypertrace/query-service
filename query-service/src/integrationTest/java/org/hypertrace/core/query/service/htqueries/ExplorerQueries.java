...
class ExplorerQueries {
   ...
   * = ? AND ( ( start_time_millis >= ? AND start_time_millis < ? ) AND ( api_boundary_type = ? AND
   * api_id != ? ) ) GROUP BY dateTimeConvert(start_time_millis,?,?,?) limit
   * 5000000=Params{integerParams={}, longParams={4=1612288800000, 5=1614686400000},
   * stringParams={0=1:MILLISECONDS:EPOCH, 1=1:MILLISECONDS:EPOCH, 2=21600:SECONDS, 3=__default,
   * 6=ENTRY, 7=null, 8=1:MILLISECONDS:EPOCH, 9=1:MILLISECONDS:EPOCH, 10=21600:SECONDS},
   * floatParams={}, doubleParams={}, byteStringParams={}} ]
   */
  static QueryRequest buildQuery1() {
    Builder builder = QueryRequest.newBuilder();

    ColumnIdentifier apiTraceCalls =
        ColumnIdentifier.newBuilder().setColumnName("API_TRACE.calls").build();
    Function apiTraceCallsFunctionCount =
        Function.newBuilder()
            .addArguments(Expression.newBuilder().setColumnIdentifier(apiTraceCalls).build())
            .setFunctionName("COUNT")
            .setAlias("COUNT_API_TRACE.calls_[]")
            .build();
    ...
    Filter filter2 =
        createFilter(
            "API_TRACE.startTime", Operator.LT, ValueType.LONG, System.currentTimeMillis());
    Filter filter3 =
        createFilter("API_TRACE.apiBoundaryType", Operator.EQ, ValueType.STRING, "ENTRY");
    Filter filter4 = createFilter("API_TRACE.apiId", Operator.NEQ, ValueType.STRING, "null");
        ...
        Function.newBuilder()
            ...
            .addArguments(
                Expression.newBuilder()
                    .setColumnIdentifier(
                        ColumnIdentifier.newBuilder().setColumnName("API_TRACE.startTime").build())
                    .build())
            .addArguments(
                Expression.newBuilder()
                    .setLiteral(
            ...
            .addArguments(
                Expression.newBuilder()
                    .setLiteral(
                        LiteralConstant.newBuilder()
                            .setValue(Value.newBuilder().setString("21600:SECONDS"))
                            .build())
                    .build())
            .build();
