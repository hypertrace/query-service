...
public class ViewDefinition {
  ...
  public static ViewDefinition parse(Map<String, Object> config, String tenantColumnName) {
    String viewName = (String) config.get("viewName");
    Map<String, PinotColumnSpec> columnSpecMap = new HashMap<>();
    final Map<String, String> fieldMap = (Map<String, String>) config.get("fieldMap");
    // todo: refactor to use attr service
    final List<String> mapFieldsList = ((List<String>) config.get("mapFields"));
  ...
  public String getKeyColumnNameForMap(String logicalName) {
    List<String> keys = findPyhsicalNameWithSuffix(logicalName, MAP_KEYS_SUFFIX);
    Preconditions.checkArgument(keys.size() <= 1);
    return keys.isEmpty() ? null : keys.get(0);
  }

  public String getValueColumnNameForMap(String logicalName) {
    List<String> keys = findPyhsicalNameWithSuffix(logicalName, MAP_VALUES_SUFFIX);
    Preconditions.checkArgument(keys.size() <= 1);
    return keys.isEmpty() ? null : keys.get(0);
  }

  private List<String> findPyhsicalNameWithSuffix(String logicalName, String suffix) {
