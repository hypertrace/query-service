...
public class PinotBasedRequestHandlerTest extends AbstractServiceTest<QueryRequest> {
  ...
  public void testInitFailure() {
    Assertions.assertThrows(
        ...
        () -> {
          Config config =
              ConfigFactory.parseMap(Map.of("name", "test", "requestHandlerInfo", Map.of()));
          new PinotBasedRequestHandler(
              config.getString("name"), config.getConfig("requestHandlerInfo"));
        });
  }
      ...
      if (config.getString("name").equals("trace-view-handler")) {
        QueryRequest request =
            QueryRequest.newBuilder()
                .setDistinctSelections(true)
                .addSelection(
                    QueryRequestBuilderUtils.createColumnExpression("Trace.does_not_exist"))
                .addSelection(
                    QueryRequestBuilderUtils.createColumnExpression("Trace.end_time_millis"))
      ...
      if (config.getString("name").equals("span-event-view-handler")) {
            ...
            QueryRequest.newBuilder()
                ...
                .setFilter(
                    Filter.newBuilder()
                        ...
                        .addChildFilter(
                            QueryRequestBuilderUtils.createEqualsFilter(
                                "EVENT.isEntrySpan", "true"))
                        .addChildFilter(
                            QueryRequestBuilderUtils.createFilter(
                                "EVENT.startTime",
                                Operator.GT,
        ...
        request =
            QueryRequest.newBuilder()
                ...
                .addSelection(QueryRequestBuilderUtils.createColumnExpression("EVENT.traceId"))
                .setFilter(
                    QueryRequestBuilderUtils.createEqualsFilter("EVENT.isEntrySpan", "false"))
                .build();
        ...
        request =
            QueryRequest.newBuilder()
                ...
                .addSelection(QueryRequestBuilderUtils.createColumnExpression("EVENT.traceId"))
                .setFilter(
                    QueryRequestBuilderUtils.createEqualsFilter("EVENT.isEntrySpan", "dummy"))
                .build();
                        ...
                        .addChildFilter(
                            QueryRequestBuilderUtils.createFilter(
                                ...
                                Expression.newBuilder()
                                    ...
                                    .build()))
                        .addChildFilter(
                            QueryRequestBuilderUtils.createEqualsFilter("EVENT.statusCode", 401L)))
                .build();
                ...
                .addChildFilter(
                    QueryRequestBuilderUtils.createFilter(
                        ...
                        Expression.newBuilder()
                            ...
                            .build()))
                .addChildFilter(
                    QueryRequestBuilderUtils.createEqualsFilter("EVENT.statusCode", "500"))
                .build();
        request =
            QueryRequest.newBuilder()
                .setDistinctSelections(true)
  ...
  public void testConvertSimpleSelectionsQueryResultSet() throws IOException {
    ...
    List<String> columnNames =
        List.of("operation_name", "service_name", "start_time_millis", "duration");
  ...
  public void testConvertAggregationColumnsQueryResultSet() throws IOException {
    ...
    List<String> columnNames =
        List.of("operation_name", "avg(duration)", "count(*)", "max(duration)");
    ResultSet resultSet = mockResultSet(4, 4, columnNames, resultTable);
    ResultSetGroup resultSetGroup = mockResultSetGroup(List.of(resultSet));
  ...
  public void testViewColumnFilterRemoval() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
      ...
      String expectedQuery =
          "Select span_id, trace_id FROM spanEventView WHERE tenant_id = ? AND ( is_entry_span = ? AND start_time_millis > ? )";
      Params params =
          Params.newBuilder()
  ...
  @Test
  public void testMaskColumnValueSelection() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
      if (!isPinotConfig(config)) {
        continue;
      }
  ...
  @Test
  public void testMaskColumnValueTableFormat() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
      if (!isPinotConfig(config)) {
        continue;
      }
  ...
  @Test
  public void testMaskMapColumnValue() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
      if (!isPinotConfig(config)) {
        continue;
      }
      ...
      String[][] resultTable =
          new String[][] {
            {
              stringify(new ArrayList<>(Arrays.asList("k1", "k2", "k3"))),
              stringify(new ArrayList<>(Arrays.asList("v1", "v2", "v3")))
            },
          };
      List<String> columnNames =
          List.of(
              "tags" + ViewDefinition.MAP_KEYS_SUFFIX, "tags" + ViewDefinition.MAP_VALUES_SUFFIX);
      ResultSet resultSet = mockResultSet(1, 2, columnNames, resultTable);
      ResultSetGroup resultSetGroup = mockResultSetGroup(List.of(resultSet));
      ...
      QueryRequest request =
          QueryRequest.newBuilder()
              .addSelection(QueryRequestBuilderUtils.createColumnExpression("EVENT.spanTags"))
              .setFilter(
                  Filter.newBuilder()
                      .setOperator(Operator.AND)
                      .addChildFilter(
  ...
  @Test
  public void testNoMaskColumnValueTenantMismatch() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
      if (!isPinotConfig(config)) {
        continue;
      }
  ...
  @Test
  public void testNoTimerangeOverlapForMasking() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
      if (!isPinotConfig(config)) {
        continue;
      }
                  ...
                  Filter.newBuilder()
                      ...
                      .addChildFilter(
                          QueryRequestBuilderUtils.createFilter(
                              "EVENT.startTime",
                              Operator.GT,
                              QueryRequestBuilderUtils.createLongLiteralValueExpression(1000)))
                      .addChildFilter(
                          QueryRequestBuilderUtils.createFilter(
                              "EVENT.startTime",
                              Operator.LT,
                              QueryRequestBuilderUtils.createLongLiteralValueExpression(2000))))
              .build();
      ExecutionContext context = new ExecutionContext("maskTenant", request);
      context.setTimeFilterColumn("EVENT.startTime");
      String expectedQuery =
  ...
  public void testViewColumnFilterRemovalComplexCase() throws IOException {
    for (Config config : serviceConfig.getConfigList("queryRequestHandlersConfig")) {
          ...
          QueryRequest.newBuilder()
              ...
              .setFilter(
                  Filter.newBuilder()
                      .setOperator(Operator.OR)
                      .addChildFilter(filter1)
                      .addChildFilter(Filter.newBuilder(filter1)))
              .build();
      ExecutionContext context = new ExecutionContext("__default", request);
      QueryCost cost = handler.canHandle(request, context);
      Assertions.assertTrue(cost.getCost() > 0.0d && cost.getCost() < 1d);
      ...
      String expectedQuery =
          "Select DISTINCT start_time_millis, span_id, trace_id FROM spanEventView WHERE tenant_id = ? AND status_code = ?";
