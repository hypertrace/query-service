  ...
  public void testEquals() {
    Params params =
        Params.newBuilder()
            ...
            .addDoubleParam(1.1d)
            .addFloatParam(0.1f)
            .addByteStringParam(ByteString.copyFrom("bytes".getBytes()))
            .build();
    ...
    Assertions.assertNotEquals(
        params, Params.newBuilder().addStringParam("test").addIntegerParam(1).build());

    params = Params.newBuilder().addStringParam("test1").addFloatParam(0.1f).build();
    Assertions.assertEquals(
        params, Params.newBuilder().addStringParam("test1").addFloatParam(0.1f).build());
    Assertions.assertNotEquals(
        params, Params.newBuilder().addFloatParam(0.1f).addDoubleParam(0.3d).build());
  }
}
